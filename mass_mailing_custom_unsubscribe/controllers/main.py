...
class CustomUnsubscribe(MassMailController):
    ...
    @route(["/mailing/<int:mailing_id>/view"], type="http", website=True, auth="public")
    def view(self, mailing_id, email=None, res_id=None, token=""):
        ...
        if mailing.exists():
            ...
            if (
                not self._valid_unsubscribe_token(mailing_id, res_id, email, str(token))
            ...
            html_markupsafe = mailing._replace_placeholder_urls(
                html_markupsafe, email, res_id
            )
